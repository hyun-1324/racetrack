...
function updateLeaderboard() {
  ...
  socket.on('update_lap_time', updatedLapTime => {
    ...
    const rowToUpdate = Array.from(leaderBoard.rows).find(
      row => row.cells[0].textContent === updatedLapTime.carNumber.toString()
    );
    ...
    tableRows.sort((a, b) => {
      ...
      return (
        TimeToMs(a.cells[3].textContent) - TimeToMs(b.cells[3].textContent)
      );
    });
