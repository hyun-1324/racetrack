...
function setButtons() {
  socket.on('upcoming_session', upcomingSessionData => {
    if (endTimeAction === 'start' || upcomingSessionData.sessionId === 0)
      return;
    ...
    for (let i = 1; i < 9; i++) {
      if (upcomingSessionData.driverNameList[i - 1] !== '') {
        ...
        button.addEventListener('click', () => {
          ...
          socket.emit(
            'lap_data',
            new lapTime(upcomingSessionData.sessionId, i, time)
          );
        });
...
function hideButtons() {
  ...
  socket.on('end_time', endTimeData => {
    if (
      endTimeData.action === 'finish' ||
      endTimeData.action === 'endSession'
    ) {
      buttongrid.style.display = 'none';
